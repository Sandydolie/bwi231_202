...
public abstract class BusinessDocument {
	...
	public void addPosition(Position pos) throws Exception {
		if (this.isReady()) {
			throw new Exception(this.getClass().getSimpleName() + " ist bereits fertig. Kann keine weiteren Positionen hinzufügen.");
		}
		else {
			this.pos.add(pos);
		}
	...
	public String toString() {
		...
		str += this.getClass().getSimpleName() + " " + this.getPrefix() + this.getNr() +
				" für " + this.getKunde() + " mit Positionen:\n";
		
		if (this.pos.size() == 0) {
			str += "Keine Positionen eingefügt.";
		}
